...
<form 
   method="POST"
   action="?/login"
   class="flex min-h-[calc(100vh-65px)]">
  ...
  <div class="flex-1 flex items-center justify-center px-8 py-16">
    <div class="w-full max-w-[400px]">
      ...
      <!-- Header -->
      <p class="font-mono text-[10px] text-[#00ff88] uppercase tracking-[0.3em] mb-3">
        // auth.login
      </p>
      <h1 class="font-display text-[36px] font-extrabold tracking-[-1.5px] leading-[1.05] mb-3">
        Welcome back,<br /><span class="text-[#00ff88]">engineer.</span>
      </h1>
      <p class="text-[13px] text-[#5a6478] mb-10">
        Don't have an account?
        <a href="/auth/signup" class="text-[#00d4ff] hover:text-[#00ff88] transition-colors no-underline font-mono text-[11px]">
          Sign up →
        </a>
      </p>
      ...
      {/if}
          ...
          >
            ...
            <span class="font-mono text-[12px] tracking-[0.08em] text-[#e8edf5]">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
          <label for="email" class="font-mono text-[10px] text-[#5a6478] uppercase tracking-[0.15em]">
            Email Address
          </label>
          <input
            id="email"
        ...
        <div class="flex flex-col gap-2">
          <div class="flex justify-between items-center">
            <label for="password" class="font-mono text-[10px] text-[#5a6478] uppercase tracking-[0.15em]">
              Password
            </label>
            <a href="/auth/forgot-password" class="font-mono text-[10px] text-[#3a4154] hover:text-[#00d4ff] transition-colors no-underline">
              Forgot?
            </a>
          </div>
          <div class="relative">
            <input
              ...
              bind:value={password}
              placeholder="••••••••"
              class="w-full bg-white/[0.04] border border-white/[0.08] px-4 py-3 pr-16
                     font-mono text-[13px] text-[#e8edf5] placeholder:text-[#3a4154]
                     focus:outline-none focus:border-[#00ff88]/50 focus:bg-white/[0.06]
                     transition-all"
            />
  ...
  <!-- ── RIGHT: Terminal art ── -->
  <div class="hidden lg:flex flex-1 border-l border-white/[0.06] items-center justify-center p-16 relative overflow-hidden">

    <!-- Ghost letters -->
    <span class="absolute font-display font-extrabold text-[220px] leading-none text-white/[0.015] select-none pointer-events-none tracking-[-8px]">
      FC
    </span>
    ...
    <div class="w-full max-w-[420px] border border-white/[0.08] bg-black/50 backdrop-blur-xl">
      <div class="flex items-center gap-2 px-4 py-[10px] bg-white/[0.04] border-b border-white/[0.08]">
        ...
        <span class="font-mono text-[10px] text-[#5a6478] mx-auto">framecode — session</span>
      </div>
      <div class="p-6 font-mono text-[11px] leading-[2]">
        <div class="flex gap-3"><span class="text-[#00ff88]">❯</span><span class="text-[#5a6478]">Authenticating user...</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#00ff88]">✓ JWT issued — expires 7d</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#00ff88]">✓ Session bound to device</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#00d4ff]">◆ Loading workspace...</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#5a6478]">  → 4 active projects found</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#5a6478]">  → GitHub repos synced</span></div>
        <div class="flex gap-3"><span class="opacity-0">❯</span><span class="text-[#00ff88]">✓ Welcome back, engineer.</span></div>
    ...
    <!-- Stats strip -->
    <div class="absolute bottom-10 left-16 right-16 grid grid-cols-3 gap-4">
      {#each [['4.2k+', 'Templates'], ['98.7%', 'Uptime'], ['12.4k', 'GitHub Stars']] as [val, lbl]}
        <div class="border border-white/[0.06] bg-white/[0.02] p-4 text-center">
          <p class="font-display text-[20px] font-bold text-[#e8edf5]">{val}</p>
          <p class="font-mono text-[9px] text-[#3a4154] uppercase tracking-[0.15em] mt-1">{lbl}</p>
        </div>
      {/each}
    </div>
